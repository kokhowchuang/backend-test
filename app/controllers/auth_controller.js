...
export function createUser(req, res, next) {
  ...
  passport.authenticate('email-signup', function (err, user, message) {
    if (err) {
      logger.warn('Unable to authenticate user in auth_controller.js. Error: ' + err);
    ...
    const token = jwt.sign({ userId: user._id }, jwtSecretKey, { expiresIn: '365d' });
...
export function authenticateUser(req, res, next) {
  ...
  passport.authenticate('email-login', function (err, user, message) {
    if (err) {
      logger.warn('Unable to authenticate user from POST routes: /auth/login in auth_controller.js. Error: ' + err);
    ...
    req.logIn(user, function (err) {
      if (err) {
        logger.warn('Unable to log user in from POST routes: /auth/login in auth_controller.js. Error: ' + err);
